...
public class Lang {
    ...
    public static boolean set(String locale) {
        if (locale.equals("") || JApp.langs.contains(locale)) {
            current.set(locale);
            return true;
        } else {
    ...
    public static void change(String locale, HttpServletResponse response, HttpServletRequest request) {
        ...
        if (set(closestLocale)) {
            if (response != null) {
                ...
                Request.setCookie(request, response, JApp.configuration.getProperty("application.lang.cookie", "JAPP_LANG"), locale, 0, true);
            }
        }
    ...
    private static void resolvefrom(HttpServletRequest request, HttpServletResponse response) {
        // Check a cookie
        String cn = JApp.configuration.getProperty("application.lang.cookie", "JAPP_LANG");
    ...
    public static void setDefaultLocale() {
        if (JApp.langs.isEmpty()) {
            set("");
        } else {
            set(JApp.langs.get(0));
        }
    }
