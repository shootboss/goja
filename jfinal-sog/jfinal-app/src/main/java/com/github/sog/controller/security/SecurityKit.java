...
public class SecurityKit {
    ...
    public static <T extends Model> void logout(HttpServletRequest req, HttpServletResponse response) {
        ...
        RequestKit.deleteCookie(req, response, COOKIE_LOGIN, true);
        // 清理Cache
        if (cookie_user != null) {
            ...
            if (user != null) {
                CacheKit.remove(LOGIN_CACHE_SESSION, LOGIN_CACHE_SESSION + user.getNumber(AppFunc.TABLE_PK_COLUMN));
            }
        }
        //清除session
    ...
    private static <T extends Model> void setLoginMember(HttpServletRequest request, HttpServletResponse response,
        ...
        request.getSession().setAttribute(LOGIN_MEMBER_ID, user.getNumber(AppFunc.TABLE_PK_COLUMN));
    ...
    public static <T extends Model> void saveMemberInCookie(T user, boolean save,
                                                            HttpServletRequest request, HttpServletResponse response) {
        String new_value = getLoginKey(user, RequestKit.getRemoteAddr(request), request.getHeader("user-agent"));
        int max_age = save ? MAX_AGE : -1;
        RequestKit.deleteCookie(request, response, COOKIE_LOGIN, true);
        RequestKit.setCookie(request, response, COOKIE_LOGIN, new_value, max_age, true);
    }
    ...
    private static <T extends Model> String getLoginKey(T user, String ip, String user_agent) {
        return encrypt(String.valueOf(user.getNumber(AppFunc.TABLE_PK_COLUMN)) + '|'
    ...
    private static CookieUser getUserFromCookie(HttpServletRequest req) {
        try {
            Cookie cookie = RequestKit.getCookie(req, COOKIE_LOGIN);
